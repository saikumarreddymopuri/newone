        ...
        if (projectId) {
            try {
                ...
                if (response.ok) {
                    ...
                    const phasesArray = Object.values(projectData.phases || {}); // Ensure phases is defined and convert to array
                    console.log('Phases Data:', phasesArray);
    
                    // Prefill checkboxes if phases are completed
                    phasesArray.forEach(phase => {
                        const checkbox = document.querySelector(`input[name="${phase.name}"]`);
                        if (checkbox && phase.status === 'completed') {
